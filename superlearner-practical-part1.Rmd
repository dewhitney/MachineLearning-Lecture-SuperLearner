...
This practical will illustrate the use of non-homogeneous ensemble methods (i.e. ensemble methods that use different algorithms).  Specifically, the SuperLearner developed by Mark Van der Laan (2007).

First change your working directory if needed (```setwd("")```) 

We will again use part of the National Health and Nutrition Examination Survey (NHANES) dataset.  We want to write the raw dataset to a data frame in our environment, and can do this with the following code: 
...
We want to keep only the complete cases in the dataset, do this using a single line of code.
...
We begin by trying different learners individually.  For example: a linear regression, lasso (glmnet), randomForest, XGBoost.  These should ideally be tested with multiple hyperparameter settings for each algorithm.  (We will do this later).
...
```
The SL.ranger fits a random forests with 500 trees, minimum node size 5, and the number of variables used to split the squared root of the number of available predictors. 
...
Now that we have a SL ensemble predictor, make predicts on the holdout data set and review the results.  This can be done using the generic ```predict()``` function, but we stipulate the option ```onlySL = TRUE``` so we do not fit algorithms in the library that had zero weight, saving computation.
...
```
We see that adding more learners slightly improves the SSE 
Adding more (without removing) will result in further improvements.
